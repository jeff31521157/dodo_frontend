...
export default class HoneycombContractWrapper {
  ...
  async deposit(params) {
    ...
    try {
      return await this.contract.methods
        .deposit(batch, pid, AmountFormat.toUint256(amount, this.decimals))
  ...
  async withdraw(params) {
    ...
    try {
      return await this.contract.methods
        .withdraw(batch, pid, AmountFormat.toUint256(amount, this.decimals))
        .send({ from: this.web3.currentAccount })
    } catch (e) {
      Logger.error(e)
      return null
  ...
  async getStakedAmount(params) {
    ...
    try {
      const { amount } = await this.contract.methods
        ...
        .call()
      return AmountFormat.fromUint256(amount, this.decimals)
    } catch (e) {
      Logger.error(e)
      return 0
    }
  ...
  async getNearestCollectableTime(params) {
    if (!this.web3.currentAccount) {
      return 0
    }

    const collectingHoney = await this.getCollectingHoney(params)
    if (collectingHoney.isZero()) {
      return 0
    }
    ...
    try {
      ...
      for (let i = 0; ; ++i) {
        const {
          ...
          .collectingInfo(batch, pid, this.web3.currentAccount, i)
          .call()
        if (!collected && collectableTime * 1000 > new Date().getTime()) {
  ...
  async getHoneyPerBlock(params) {
    ...
    try {
      ...
      const { allocPoint } = await this.contract.methods
        .poolInfo(batch, pid)
        .call()
      return new BigNumber(honeyPerBlock)
        .times(allocPoint)
        .div(totalAllocPoint)
        .div(10 ** 18)
    } catch (e) {
  ...
  async getTotalShares(params) {
    const { batch, pid } = params
    try {
      const { totalShares } = await this.contract.methods
        .poolInfo(batch, pid)
        .call()
      return new BigNumber(totalShares).div(10 ** this.decimals)
    } catch (e) {
      Logger.error(e)
      return new BigNumber(0)
    }
