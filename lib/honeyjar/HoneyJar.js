...
export default class HoneyJar {
  constructor(params) {
    ...
    this.strategyAPY = 0
    this.honeycombStages = honeycombStages
    this.latestHoneycombStage = honeycombStages[honeycombStages.length - 1]
    this.honeycombKey = this.latestHoneycombStage + '-' + hTokenSymbol
    this.web3 = web3
    this.axios = axios
  ...
  async calculateAPY() {
    try {
      const res = await this.axios.$get(
        `https://api.myhoney.finance/honeyjar/info/${this.hTokenSymbol.toLowerCase()}`
      )
      if (res.data) {
        if (res.data.growth30) {
          this.strategyAPY = ((res.data.growth30 - 1) / 30) * 365 * 100
        } else if (res.data.growth7) {
          this.strategyAPY = ((res.data.growth7 - 1) / 7) * 365 * 100
        } else if (res.data.growth3) {
          this.strategyAPY = ((res.data.growth3 - 1) / 3) * 365 * 100
        } else if (res.data.growth1) {
          this.strategyAPY = (res.data.growth1 - 1) * 365 * 100
        } else if (res.data.rate) {
          this.strategyAPY = (res.data.rate - 1) * 365 * 100
        }
      }
    } catch (e) {
      Logger.error(e)
